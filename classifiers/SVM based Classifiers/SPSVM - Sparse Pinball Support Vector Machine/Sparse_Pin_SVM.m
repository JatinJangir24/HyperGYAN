...
function [Y_Predicted, training_time] = Sparse_Pin_SVM(TestX,DataTrain2,FunPara)

X1_Train = DataTrain2(DataTrain2(:,end) == 1, 1:end-1);
X2_Train = DataTrain2(DataTrain2(:,end) == -1, 1:end-1);
Y_Train = [DataTrain2(DataTrain2(:,end) == 1, end); DataTrain2(DataTrain2(:,end) == -1, end)];
X_Test = TestX(:, 1:end-1);
Y_Test = TestX(:, end);
c = FunPara.c;
epsilon = FunPara.epsilon;
tau = FunPara.tau;
